...
# ---------------------------------------------------------------------------
# Utilidades de umbral (threshold tuning)
# ---------------------------------------------------------------------------

def tune_threshold(model, X_val, y_val, metric='f1', thresholds=None):
    """
    Encuentra el umbral de decisión óptimo para un modelo ya entrenado.

    Evalúa un rango de umbrales aplicados a las probabilidades del modelo y
    devuelve el que maximiza la métrica indicada.

    Args:
        model:      Modelo sklearn con predict_proba o decision_function.
        X_val:      Features del conjunto de validación.
        y_val:      Etiquetas reales del conjunto de validación.
        metric:     Métrica a maximizar: 'f1' (default), 'precision', 'recall'.
        thresholds: Array de umbrales a evaluar. Si None se usa
                    np.arange(0.10, 0.91, 0.01).

    Returns:
        tuple: (best_threshold: float, best_score: float)
    """
    ...
    if hasattr(model, 'predict_proba'):
        ...
        return 0.5, None   # fallback: el modelo no soporta probabilidades
    ...
    score_fn = metric_fns.get(metric, metric_fns['f1'])
...
def predict_with_threshold(model, X, threshold=0.5):
    """
    Genera predicciones binarias aplicando un umbral personalizado.

    Args:
        model:     Modelo sklearn con predict_proba o decision_function.
        X:         Features sobre las que predecir.
        threshold: Umbral de decisión (default 0.5).

    Returns:
        np.ndarray: Predicciones binarias (0 / 1).
    """
    if hasattr(model, 'predict_proba'):
        ...
        return model.predict(X)   # fallback directo
...
def train_multiple_models(X_train, y_train, X_test, y_test,
                          use_class_weight=True, sampler=None):
    """
    ...
    Args:
        ...
        sampler: Objeto sampler compatible con imblearn (p.ej. SMOTE,
                 RandomUnderSampler). Si se provee se aplica únicamente
                 sobre X_train/y_train antes de entrenar los modelos.
    ...
    # Aplicar sampler si se proporciona (sólo en train)
    if sampler is not None:
        print(f"\n🔧 Aplicando sampler: {sampler.__class__.__name__}...")
        X_train, y_train = sampler.fit_resample(X_train, y_train)
        n0 = int((y_train == 0).sum())
        n1 = int((y_train == 1).sum())
        print(f"✓ Dataset rebalanceado: {X_train.shape[0]} muestras")
        print(f"   Clase 0: {n0} ({n0/len(y_train)*100:.1f}%)")
        print(f"   Clase 1: {n1} ({n1/len(y_train)*100:.1f}%)")
...
def main_training_pipeline(X_train, y_train, X_test, y_test,
                           output_dir='../../results/',
                           sampler=None,
    ...
    Args:
        ...
        sampler: Sampler imblearn opcional para rebalancear clases en train.
        tune_thresholds: Si True calcula el umbral óptimo por modelo usando
                         X_test/y_test y añade Optimal_Threshold al DataFrame.
        threshold_metric: Métrica usada para el tuning ('f1', 'precision',
                          'recall'). Default 'f1'.
    ...
    models_dict, results_df = train_multiple_models(
        X_train, y_train, X_test, y_test, sampler=sampler
    )
    
    # Tuning de umbrales (opcional)
    if tune_thresholds:
        print(f"\n🔍 Optimizando umbrales de decisión (métrica: {threshold_metric})...")
        thresholds_col = []
        scores_col = []
        for model_name in results_df['Model']:
            model = models_dict.get(model_name)
            if model is None:
                thresholds_col.append(0.5)
                scores_col.append(np.nan)
                continue
            t, s = tune_threshold(model, X_test, y_test,
                                  metric=threshold_metric)
            thresholds_col.append(round(t, 4))
            scores_col.append(round(s, 4) if s is not None else np.nan)
            print(f"  • {model_name}: threshold={t:.2f}  {threshold_metric}={s:.4f}")
        results_df['Optimal_Threshold'] = thresholds_col
        results_df[f'Best_{threshold_metric.upper()}'] = scores_col
        print("✓ Umbrales optimizados calculados")
