...
class DataDriftDetector:
    ...
    def fit(self, baseline_data, feature_names: List[str] = None):
        """
        Establece los datos de referencia (baseline) para comparaciones futuras
        
        Args:
            baseline_data: DataFrame o array con los datos de referencia (ej: train set)
            feature_names: Lista de nombres de features (opcional, requerido si se pasa array)
        """
        # Convert numpy arrays to DataFrame if needed
        if not isinstance(baseline_data, pd.DataFrame):
            if feature_names is None:
                raise ValueError("feature_names debe proporcionarse cuando baseline_data es un array")
            baseline_data = pd.DataFrame(baseline_data, columns=feature_names)
    ...
    def detect_drift(self, current_data, 
        ...
        Args:
            current_data: DataFrame o array con datos actuales
        ...
        # Convert array to DataFrame if necessary
        if not isinstance(current_data, pd.DataFrame):
            if self.feature_names is None:
                raise ValueError("feature_names no está definido, no se puede convertir array a DataFrame")
